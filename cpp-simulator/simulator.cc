...
plot_data_struct run_simulation(){
  ...
  auto nodes = init_nodes(); //TODO: In init_nodes, read individuals.json
...
//TODO: Assign Individuals campus 
  ...
  for(count_type time_step = 0; time_step < GLOBAL.NUM_TIMESTEPS; ++time_step){
    ...
    update_interaction_spaces_lambda(nodes, interaction_spaces, day);
    update_individuals_lambda(nodes, interaction_spaces, day);
    /*
	  if(node_update_status.new_infection){
		++num_new_infections;
		++num_total_infections;
