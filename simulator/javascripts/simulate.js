...
// Populate the train travel
for (var i=0; i<nodes.length; i++){
  if(Math.random < p){
    node[i]['tr'] = 1
  }
}
function update_train_lambda(node,nodes){
  var sum =0;
  for (var i=0; i<nodes.length; i++){
  sum+= nodes[i]['infective']* nodes[i]['tr']*euclidean(nodes[i]['workplace'], nodes[i]['home'])*6371
  }
  sum=sum/Math.sum((euclidean(nodes['workplace'], nodes['home'])*6371)*nodes['tr'])*euclidean(node['workplace'], node['home'])*6371*N_PER*C_TRAIN
  return sum
}
...
function update_lambdas(node,homes,workplaces,communities,nodes,cur_time){
  ...
  if(cur_time % 2 == 1)
	{node['lambda'] = node['lambda_incoming'][0]+node['lambda_incoming'][1]+node['lambda_incoming'][2]+update_train_lambda(node,nodes);}//Add trains at odd times only
	else{
	  node['lambda'] = node['lambda_incoming'][0]+node['lambda_incoming'][1]+node['lambda_incoming'][2]
	}
	
//Math.sum(nodes[i]['infective'])/NUM_PEOPLE*N_PER*C_TRAIN*N_TRAVEL Train extra added	
}
/*
